...
fu = 600 # Uniform number of frames
    ...
    for i_s, skeleton in enumerate(tqdm(s)):  # pad
        ...
        for i_p, person in enumerate(skeleton):
            ...
            for i_f, frame in enumerate(person):
                if frame.sum() == 0:
                    if person[i_f:].sum() == 0:             
                        fo = i_f + 1   # fo:Original frames
                        DM = np.zeros((fo-1, V, C)) # DM:Displacement matrix
                        FF = np.zeros((fu, V, C)) # FF:Fill frame matrix
                        if fu % fo != 0:
                            ff = fu // fo  # ff:The number of frames that need to be filled between frames
                            fd = (fo * (ff + 1)) - fu # fd:Number of frames that need to be discarded
                        else:
                            ff = fu // fo - 1
                            fd = 0  
                        ...
                        for t in range(fo):
                            for v in range(V):
                                for c in range(C):
                                    if t < i_f:
                                        DM[t, v, c] = person[t + 1, v, c] - person[t, v, c]
                                        Nid = DM[t, v, c] / (ff + 1)  # Nid:New inter-frame displacement
                                    else:
                                        Nid = DM[t-1, v, c] / (ff + 1)  # Nid:New inter-frame displacement
                                        
                                    for f in range(ff+1):
                                        pnl = 0 # The proportion of noise level
                                        Nc = f * Nid * (1+pnl) + person[t, v, c]  # Nc:New coordinates
